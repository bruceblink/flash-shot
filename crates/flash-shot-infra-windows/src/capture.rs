...
use crate::display::{DisplayInfo, DisplayProvider, SystemDisplayProvider, virtual_desktop_bounds};
use flash_shot_domain::domain::geometry::PhysicalRect;

// Keep the old platform path as a compatibility re-export while the frame data belongs to the
// platform-independent domain crate.
pub use flash_shot_image::{CaptureFrame, PixelColor, PixelFormat};
...
mod platform {
    ...
    fn draw_cursor(dc: HDC, bounds: PhysicalRect) -> io::Result<()> {
        ...
        if cursor.flags & CURSOR_SHOWING == 0
            ...
            || !bounds.contains(flash_shot_domain::domain::geometry::PhysicalPoint {
...
mod tests {
    ...
    use crate::display::{DisplayInfo, DisplayProvider, DisplayRotation};
    #[cfg(windows)]
    use crate::display::{SystemDisplayProvider, virtual_desktop_bounds};
