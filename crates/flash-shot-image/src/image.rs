...
use crate::{CaptureFrame, PixelFormat};
...
fn draw_text_annotation(
    ...
    for character in content.chars() {
        let Some(glyph) = font.glyph_for_char(character) else {
            pen_x += flash_shot_domain::domain::annotation::TEXT_ANNOTATION_ADVANCE as f32;
            continue;
        };
...
/// Replaces the destination with an encoded image while preserving the platform's atomicity rule.
#[cfg(windows)]
...
mod tests {
    use super::*;
    use flash_shot_domain::domain::annotation::{
