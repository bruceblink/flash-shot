...
ownership, and per-user auto-start state now live in `flash-shot-infra-windows`. `flash-shot-app::platform`
keeps compatibility exports while inspection, process, and file-system implementations remain in the
application crate until their contracts are isolated. Keep shared traits and product errors in
