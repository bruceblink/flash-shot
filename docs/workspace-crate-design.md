...
virtual desktop composition, global shortcut registration, tray event/menu handling, and clipboard image/text
ownership now live in `flash-shot-infra-windows`. `flash-shot-app::platform` keeps compatibility exports while
inspection, process, and file-system implementations remain in the application crate until their contracts are
isolated. Keep shared traits and product errors in
