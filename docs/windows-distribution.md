...
```powershell
.\scripts\verify-portable-package.ps1 -ArchivePath "dist\FlashShot-0.1.3-windows-x86_64.zip"
```
...
```powershell
.\scripts\smoke-portable-startup.ps1 -ArchivePath "dist\FlashShot-0.1.3-windows-x86_64.zip"
```
...
```powershell
.\scripts\smoke-installer.ps1 -InstallerPath "dist\FlashShot-0.1.3-windows-setup.exe"
```
...
The repository packages a Windows release when a `v<major>.<minor>.<patch>` tag is pushed, or when the `Release` workflow is manually run for an existing tag. The tag must exactly match the Cargo package version; for example, `Cargo.toml` version `0.1.3` requires tag `v0.1.3`:
...
```powershell
$tag = "v0.1.3" # Replace with the actual new draft tag.
.\scripts\verify-github-release.ps1 -Tag $tag -RequireDraft
```

With `-RequireDraft`, it also rejects a release that has already been published, keeping this check as an explicit pre-publish gate. It deletes its temporary download directory after a successful or failed check. Pass `-OutputDirectory target\release-v0.1.3` to retain the downloaded assets for manual inspection. The GitHub runner proves the unsigned setup, install, startup, and uninstall path on its fresh account; the operator still manually checks screenshot capture, annotation, save/copy, and the FFmpeg recording path before publishing the draft.
