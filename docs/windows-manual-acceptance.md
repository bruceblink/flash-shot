...
| 2026-08-10 | `c30c34a` | 333 项库测试、严格 Clippy、格式检查和 `recording-acceptance` 验收探针通过；显示器、窗口、区域三种目标均生成 H.264 MP4，并由 `ffprobe` 验证时长、编码和尺寸；暂停/恢复/停止事件均观察到，录制结束后无 FFmpeg 子进程。 | 待执行 | 生产录屏后端证据位于 `target/ui-acceptance/recording-p2-display-20260810.json`、`target/ui-acceptance/recording-p2-window-20260810.json` 与 `target/ui-acceptance/recording-p2-region-20260810.json`；完整 UI 手工矩阵仍待执行。窗口验收使用可见的 `Flash Shot` 设置窗口，截图证据为 `target/ui-acceptance/recording-window-settings-keepalive.png`。 |
