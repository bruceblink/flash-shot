...
| 2026-08-10 | `57980ca` | 341 项库测试、严格 Clippy、格式检查和滚动控制器回归测试通过；辅助滚动按钮明确显示 `Scroll down + capture`，等待态显示 `Scrolling...`。 | 待执行 | 文案与当前固定的向下滚轮方向一致，避免用户误判滚动方向；真实滚动 UI、双屏/混合 DPI 和录屏手工矩阵仍待执行。 |
