...
| 2026-08-10 | `current-environment-followup` | 当前 Release 环境探针通过：检测到单块 `DISPLAY1`（2560x1440、DPI 96、scale 1.0），FFmpeg 9.0 可用并支持 `gdigrab`、窗口和区域输入；识别就绪探针 schema 2 通过只读模式。 | 待执行 | 机器可读证据位于 `target/windows-acceptance-environment-20260810-followup.json` 与 `target/ui-acceptance/recognition-acceptance-followup.json`。当前未安装 Tesseract、未配置翻译端点，因此 OCR/翻译真实 UI、150%/200% 缩放、双屏和完整录屏 UI 仍不标记为通过。 |
