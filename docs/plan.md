...
| W5 | 选区锚定、HUD 与布局快照 | 进行中（2026-09-24；确定性边界矩阵与单屏 100% 窄边缘原生输入已通过） | 已统一工作区布局快照；零面积拒绝、1px、全屏和四边贴边的确定性矩阵通过，右下窄选区真实 More/Mark 输入已通过；对应真实键鼠边界矩阵仍待补齐 |
...
全部通过。该证据关闭确定性布局子项，不替代这些边界场景的真实鼠标、键盘、导出像素和清理验收；150%/200% DPI 与多屏
继续按 D1 暂缓，因此 W5 保持进行中。
...
**W 路线执行顺序**：B1 外部失败恢复、B4 当前 HEAD 的真实标注输入、W1 工作区视觉基础与共享控件、W2 主工具栏重排、W3 上下文样式栏以及 W4 工具组与 More 浮层均已完成；W5 已完成布局实现和右下窄选区的单屏 100% 原生输入子集，下一步补齐零/极小、全屏和四边选区矩阵，再进入 W6。
...
**2026-09-24 后续执行顺序**：完成 `w5-layout-snapshot` 的验证后，将其合并并推送到 `main`，确认本地/远程提交一致、
工作区干净且没有关联 worktree，再删除已合并的本地与远程分支。后续开发直接以最新 `main` 为唯一基线，不再创建 `dev`；
每个独立可验收功能仍须先完成风险匹配的测试和 UI 验收，再以一次独立 Conventional Commit 提交并立即推送 `main`。接下来先
补齐 W5 的零/极小、全屏和四边选区真实输入、导出像素与清理矩阵，再按 W6、U1-U4、M1 最小职责拆分和 `0.2.0` 发布收尾
的顺序推进；不得用确定性布局或单个静态探针替代原生验收。
...
下一步固定补齐 **W5 的零/极小、全屏和四边选区矩阵**；若当前 Windows 会话缺少所需权限、消费者或 FFmpeg，记录阻塞证据，
先完成可执行的确定性测试，不把环境缺失写成通过，也不越过 W5 扩展新功能。W5 通过后创建并验证 `dev`，再进入 W6；
