...
| W6 | 双主题、双语、DPI 与真实输入验收 | 进行中（2026-09-25；第一切片完成） | `run-w6-workspace-matrix.ps1` 已用同一 Release 生成 84 个 420x420、520x640、980x760 的双主题/双语覆盖层状态截图，84/84 通过 DPI 96/scale 1.0 和 metadata 校验；真实鼠标键盘、高 DPI 与多屏仍保持未完成或 D1 暂缓 |
...
**W 路线执行顺序**：B1 外部失败恢复、B4 当前 HEAD 的真实标注输入、W1 工作区视觉基础与共享控件、W2 主工具栏重排、W3 上下文样式栏、W4 工具组与 More 浮层以及 W5 选区边界矩阵均已完成当前环境范围；W6 静态状态矩阵第一切片已完成，下一步继续 W6 的真实输入验收。
...
**2026-09-25 后续执行顺序**：完成 `w5-selection-boundary-matrix` 的验证后，将其合并并推送到 `main`，确认本地/远程提交一致、
工作区干净且没有关联 worktree，再删除已合并的本地与远程分支。后续开发以最新 `main` 为起点同步 `dev`，功能分支先合并并验证
`dev`，再合并并推送 `main`；每个独立可验收功能仍须先完成风险匹配的测试和 UI 验收，再以一次独立 Conventional Commit 提交并立即推送当前目标分支。接下来进入
W6 的 100% 单屏双主题/双语状态矩阵，随后按可用硬件补充 DPI 证据，再按 U1-U4、M1 最小职责拆分和 `0.2.0` 发布收尾
