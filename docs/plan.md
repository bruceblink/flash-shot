...
| W6 | 双主题、双语、DPI 与真实输入验收 | 进行中（下一切片） | 以同一 Release 执行 420x420、520x640、980x760 的双主题/双语状态矩阵，并覆盖真实鼠标键盘动作；当前 100% 环境先完成，未具备的高 DPI 与多屏显式保留为 D1 暂缓 |
...
**W 路线执行顺序**：B1 外部失败恢复、B4 当前 HEAD 的真实标注输入、W1 工作区视觉基础与共享控件、W2 主工具栏重排、W3 上下文样式栏、W4 工具组与 More 浮层以及 W5 选区边界矩阵均已完成当前环境范围；下一步进入 W6 的双主题/双语状态矩阵。
...
每个独立可验收功能仍须先完成风险匹配的测试和 UI 验收，再以一次独立 Conventional Commit 提交并立即推送 `main`。接下来进入
...
下一步固定进入 **W6 的双主题、双语和真实输入状态矩阵**；若当前 Windows 会话缺少所需 DPI 硬件，记录 D1 暂缓证据，
