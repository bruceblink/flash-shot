...
本次复审基于 `main` 提交 `8356640`（2026-09-02）以及 workspace 中的五个 crate。复审范围包括 Cargo
...
**目标**：把高体积、可选依赖和变化较快的能力从核心截图主链中隔离，降低单一安装包和长期维护成本，同时保留统一的
截图会话、取消、权限和资源清理规则。

**边界**：主程序先提供插件宿主、版本化插件清单、权限声明、进度/取消/错误接口和有界帧流；录屏与 GIF 先以
内置插件验证。首期不承诺第三方市场、任意 Rust 动态库 ABI、网络插件或跨平台插件。

**顺序**：B1-B4、U1-U4 和 M1 的行为证据稳定后，先冻结 `FrameStream`、`ExportRequest` 和插件报告 schema；
再将现有 FFmpeg 录屏适配为内置录屏插件，增加消费有界帧流的 GIF 导出插件；最后评估以独立进程 IPC/stdio 运行的
第三方插件。插件不能直接操作 HWND、全局快捷键、系统剪贴板或 `CaptureSession` 内部状态，必须通过宿主接口申请。

**验收**：清单版本不兼容时明确拒绝；插件崩溃、超时、取消和输出失败均能回收子进程、临时文件和宿主任务；未安装
可选插件不影响截图主链启动；录屏/GIF 产物、帧率/时长/大小限制、English/简体中文错误反馈和安装包增量均有独立
Release 证据。
