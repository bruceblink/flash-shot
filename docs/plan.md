...
**W 路线执行顺序**：B1 外部失败恢复、B4 当前 HEAD 的真实标注输入、W1 工作区视觉基础与共享控件以及 W2 主工具栏重排均已完成；下一步进入 W3 上下文样式栏。
W0 的路线文档切片已完成，随后严格按 W3、W4、W5、W6 顺序推进。W1-W4 新增的可见文案同时登记到 `UiText`；U1 继续负责非截图工作区的
...
下一步固定从 **W3 上下文样式栏** 开始；若当前 Windows 会话缺少所需权限、消费者或 FFmpeg，记录阻塞证据，
先完成可执行的确定性测试，不把环境缺失写成通过，也不越过 W3 扩展新功能。W1-W6 完成后进入 M1；W0 只代表本次公开研究
和路线文档已完成，W2 的静态 Release 证据也不替代 W6 的真实输入验收。
