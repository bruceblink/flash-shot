...
| W5 | 选区锚定、HUD 与布局快照 | 进行中（2026-09-19；布局快照基础） | 让工具栏行、样式行和选区边缘在小选区、边缘选区及 DPI 场景稳定协同 |
...
**本次进展（2026-09-19）**：新增 `WorkspaceLayoutSnapshot`，在一次布局计算中统一记录安全边距、`PreviewTransform`
...
该进展只覆盖布局计算与回归保护，尚未完成零/极小选区的完整矩阵、真实鼠标键盘、150%/200% DPI 和多屏原生验收，W5 仍保持进行中。
...
**W 路线执行顺序**：B1 外部失败恢复、B4 当前 HEAD 的真实标注输入、W1 工作区视觉基础与共享控件、W2 主工具栏重排、W3 上下文样式栏以及 W4 工具组与 More 浮层均已完成；W5 已进入布局快照基础实现，下一步继续补齐 W5 的边缘/HUD 矩阵和原生验收，再进入 W6。
