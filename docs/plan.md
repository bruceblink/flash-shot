...
| W5 | 选区锚定、HUD 与布局快照 | 进行中（2026-09-19；布局实现与单屏 100% 窄边缘原生输入已通过） | 已统一工作区布局快照并完成右下窄选区的真实 More/Mark 输入与清理验收；零/极小选区、全屏和四边矩阵仍待补齐 |
...
当前仍缺零/极小选区、全屏选区和顶部/底部/左侧/右侧的统一矩阵；150%/200% DPI 与多屏继续按 D1 暂缓，W5 保持进行中。
...
**2026-09-24 后续执行顺序**：当前远程没有 `dev` 分支，因此继续在已推送的 `w5-layout-snapshot` 上收口 W5；W5 的剩余
确定性测试、同一 Release 的原生输入和清理证据通过后，创建 `dev` 作为后续开发与集成基线并推送。随后从 `dev` 创建
W6 验收分支，按 W6、U1-U4、M1 最小职责拆分和 `0.2.0` 发布收尾的顺序推进。`main` 只接收经过 `dev` 完整验证的结果，
不得直接用当前本地领先提交或单个静态探针跳过集成验证。
