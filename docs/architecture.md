...
| Windows 基础设施库 | Windows Infrastructure Crate / `flash-shot-infra-windows` | 显示器枚举、虚拟桌面边界、Windows 屏幕捕获、全局快捷键、托盘事件、剪贴板和自启动实现；后续承载其他原生服务 | 不是应用用例、GPUI 界面或第二个程序入口 |
...
crates/flash-shot-infra-windows/src/        显示器枚举、屏幕捕获、全局快捷键、托盘、剪贴板和自启动
