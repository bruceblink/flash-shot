...
- `crates/flash-shot-infra-windows/src/` 中的显示器、捕获、快捷键、剪贴板、托盘和窗口检查实现，在非 Windows 平台会明确返回 `Unsupported`。`flash-shot-app::platform` 只保留兼容导出，当前运行时不能在 Linux 上完成截图主链路。
...
每个阶段都需要真实桌面环境的截图、复制、取消、重连和多显示器手工验收，以及对应的平台接口约定测试。完成这些证据前，发行资产和产品文档继续只承诺 Windows。
