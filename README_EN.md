...
`flash-shot-infra-windows` owns display enumeration, Windows screen capture, global shortcuts, the tray, the clipboard, auto-start, directory opening, process lifecycle, window control, cursor positioning, and assisted wheel input, `flash-shot-app` owns
application workflows, GPUI, and infrastructure composition, and `crates/flash-shot-bin` is the
